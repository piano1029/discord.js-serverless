 ...
 * @extends {BaseClient}
 */
class InteractionURLClient extends BaseClient {
  /**
   * @param {InteractionURLClientOptions} options Options for the client
   */
  constructor(options) {
    super(options);
    ...
    /**
     * Map containing callbacks
     * @type {Map<string, Function>} Map of callbacks
     */
  ...
  /**
   * Data ingestion function for the client
   * @param {Object} data Data to ingest
   * @param {string} rawData Raw body data
   * @param {string} Signature Discord provided signature
   * @param {string} SignatureTimestamp Discord provided signature timestamp
   * @returns {any} Data that should be sent back to Discord. If this returns `false`, it must be met with a 401 Unauthorized.
   */
  async ingest(data, rawData, Signature, SignatureTimestamp) {
    return new Promise((resolve, reject) => {
      const isVerified = nacl.sign.detached.verify(
        Buffer.from(SignatureTimestamp + rawData),
      ...
      new InteractionCreate(this).handle(data);
    });
  }
   ...
   */
  isReady() {
    return true; // This.ws.status === Status.Ready;
  }
...
module.exports = InteractionURLClient;
